...
int main (int argc, char *argv[])
{
	...
	neon_green.green= 52428;
	neon_green.blue=0;
	neon_green.red=26214;
...
void back_label(GtkWidget *back_button_f){

gtk_label_set_text (GTK_LABEL(song_label), ent[song_index]->d_name);

}



void next_label(GtkWidget *next_button_f){}
...
void list_init(){
  ...
  GtkWidget *button;
  GtkWidget *scroll;
	GtkWidget *back_button_f;
	GtkWidget *next_button_f;
 ...
 back_button_f = gtk_button_new_with_label(NULL);
 next_button_f = gtk_button_new_with_label(NULL);

	 
	 gtk_box_pack_start(GTK_BOX(vbox), hbox, FALSE, FALSE, 5);
  	 gtk_box_pack_start(GTK_BOX(hbox), back_button_f, TRUE, TRUE, 5);
	 gtk_box_pack_start(GTK_BOX(hbox), next_button_f, TRUE, TRUE, 5);
  	 gtk_box_pack_start(GTK_BOX(hbox), button, FALSE, FALSE, 5);

	GtkWidget *plat_image=gtk_image_new_from_file ("./media/play.png");
	gtk_button_set_image (GTK_BUTTON(button), plat_image);

	GtkWidget *b_image=gtk_image_new_from_file ("./media/tunedown.png");
	gtk_button_set_image (GTK_BUTTON(back_button_f), b_image);

	GtkWidget *n_image=gtk_image_new_from_file ("./media/tuneup.png");
	gtk_button_set_image (GTK_BUTTON(next_button_f), n_image);


  g_signal_connect(button, "clicked", G_CALLBACK(mp3_init), NULL);
	g_signal_connect(back_button_f, "clicked", G_CALLBACK(back_label), NULL);
	g_signal_connect(next_button_f, "clicked", G_CALLBACK(next_label), NULL);
  ...
  closedir (dir);
	...
	gtk_widget_modify_bg(GTK_WIDGET(next_button_f), GTK_STATE_NORMAL, &black);
	gtk_widget_modify_bg(GTK_WIDGET(next_button_f), GTK_STATE_ACTIVE, &neon_green);

	gtk_widget_modify_bg(GTK_WIDGET(back_button_f), GTK_STATE_NORMAL, &black);
	gtk_widget_modify_bg(GTK_WIDGET(back_button_f), GTK_STATE_ACTIVE, &neon_green);
